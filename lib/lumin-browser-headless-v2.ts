...
async function cleanup(name: string, token: string, projectId: string, teamId: string) {
  try {
    const q = new URLSearchParams({ projectId, teamId })
    await fetch(`https://api.vercel.com/v2/sandboxes/${encodeURIComponent(name)}?${q}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(10_000) })
  } catch {}
}
  ...
  try {
    ...
    try {
      ...
      const run = await runCommand({
        ...
        env: { LUMIN_ACTION: JSON.stringify(input.action), LUMIN_COOKIES: JSON.stringify(Array.isArray(input.cookies) ? input.cookies : []) },
      })
      ...
      if (created) await cleanup(name, token, projectId, teamId)
    }
  } catch (error: any) {
