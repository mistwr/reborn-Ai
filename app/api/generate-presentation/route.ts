...
function getSlideImage(query: string, index: number, seed?: number): string {
  const s = seed ?? index * 17 + 42
  const sources = [
    `https://source.unsplash.com/800x600/?${encodeURIComponent(query)}&sig=${s}`,
    `https://picsum.photos/seed/${s}/800/600`,
  ]
  // Alternate sources per slide
...
function buildHtml(
  ...
  const isLight = ["corporate", "creative", "minimal", "pastel"].includes(templateId)

  const slides = slidesData.map((slide, i) => {
    const isTitle = i === 0
    const isEnd = i === slidesData.length - 1
    const imgUrl = getSlideImage(slide.imageQuery || slide.title, i)
...
.split-text{
  flex:1;padding:clamp(30px,5vw,80px);
...
.split-slide.reverse .split-img::before{
...
h2{
  ...
  color:var(--text);line-height:1.2;margin-bottom:2rem;
}
ul{list-style:none}
li{
...
.slide-num{
  ...
  font-size:.75rem;opacity:.35;letter-spacing:.05em;
}

/* Progress & controls */
#progress-bar{
  position:fixed;bottom:0;left:0;height:3px;
  background:var(--accent);transition:width .4s;z-index:1000;
}
.ctrl{
...
#fs-hint{
  ...
  opacity:.3;z-index:1000;cursor:pointer;
}
#fs-hint:hover{opacity:.7}

@media(max-width:768px){
  .split-img{display:none}
  .title-image{width:100%;opacity:.3}
  .title-content{max-width:100%;padding:5vw 6vw}
  .split-text{padding:30px 24px}
  .text-content{padding:30px 24px}
}
@media print{
  .slide{position:static;opacity:1;transform:none;page-break-after:always;height:100vh}
  #progress-bar,.ctrl,#thumb-panel,#fs-hint{display:none}
}
</style>
</head>
<body>
...
Slide 1: title slide (title = main topic, content = [one-line tagline], imageQuery = topic keyword)
Slides 2–${slideCount - 1}: content slides with clear titles and 3-5 bullet points each
...
[{"title":"...","content":["..."],"imageQuery":"...","layout":"image-right","notes":"..."}]`,
    ...
    try {
      ...
      slidesData = [
        { title: prompt, content: ["Gerado por Reborn AI"], imageQuery: "presentation professional" },
        { title: "Conclusão", content: ["Obrigado!"], imageQuery: "thank you" },
      ]
    }
