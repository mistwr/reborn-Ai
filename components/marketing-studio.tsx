...
export function MarketingStudio() {
  ...
  const buildPrompt = () => {
    ...
    const stylePrompt = STYLE_PROMPTS[visualStyle]
    const platformInfo = PLATFORMS.find((p) => p.id === platform)!

    let prompt = `${contentType} design for ${platformInfo.label}, ${stylePrompt}`
    if (mainText) prompt += `, headline text "${mainText}"`
    if (subText) prompt += `, subtext "${subText}"`
    if (brandName) prompt += `, brand name "${brandName}"`
    if (cta) prompt += `, call to action "${cta}"`
    prompt += `, color ${primaryColor}, ${size.w}x${size.h}px, professional marketing material, high quality graphic design`
  ...
  const generate = async () => {
    ...
    // Use Pollinations directly for marketing (fast + free)
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=${size.w}&height=${size.h}&nologo=true&seed=${seed}&model=flux`

    const result: MarketingResult = { url, prompt, type: contentType, platform, style: visualStyle }
  ...
  const regenerate = async () => {
    if (!selected) return
    setIsGenerating(true)
    const size = SIZES[selected.type]
    const seed = Math.floor(Math.random() * 9999999)
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(selected.prompt)}?width=${size.w}&height=${size.h}&nologo=true&seed=${seed}&model=flux`
    const result: MarketingResult = { ...selected, url }
    setResults((prev) => [result, ...prev])
    setSelected(result)
    setIsGenerating(false)
  }
      ...
      <div className="max-w-5xl mx-auto w-full p-4 pb-10 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Cria posts, banners, stories e thumbnails profissionais com IA</p>
          </div>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <div className="space-y-4">
                ...
                <Card className="p-4 space-y-3">
                  ...
                  <p className="text-[10px] text-muted-foreground">
                    Tamanho: {SIZES[contentType].w}x{SIZES[contentType].h}px
                  </p>
                </Card>
                    ...
                    {VISUAL_STYLES.map(({ id, label, preview, desc }) => (
                      <button
                        ...
                        className={`rounded-xl border overflow-hidden transition-all ${
                          visualStyle === id ? "border-primary ring-1 ring-primary shadow-sm" : "border-border hover:border-primary/40"
                        }`}
                      >
                        <div className={`h-8 bg-gradient-to-r ${preview}`} />
                        <div className="px-2 py-1.5 text-left">
                          <div className="text-xs font-semibold text-foreground">{label}</div>
                          <div className="text-[9px] text-muted-foreground leading-tight">{desc}</div>
                        </div>
                      </button>
                    ))}
                  </div>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {["#6366f1", "#ec4899", "#10b981", "#f59e0b", "#ef4444", "#3b82f6"].map((c) => (
                            <button
                              ...
                              className={`w-5 h-5 rounded-full border transition-all ${primaryColor === c ? "border-foreground scale-110" : "border-transparent"}`}
              ...
              <div className="space-y-4">
                <Card className="p-4 space-y-3">
                  ...
                  {selected ? (
                    <>
                      ...
                      <div className="flex flex-wrap gap-2 items-center">
                        ...
                        <Badge variant="outline" className="text-[10px] capitalize">
                          {PLATFORMS.find((p) => p.id === selected.platform)?.label}
                        </Badge>
                        <Badge variant="outline" className="text-[10px] capitalize">
                          {VISUAL_STYLES.find((s) => s.id === selected.style)?.label}
                        </Badge>
                      </div>

                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" className="flex-1" onClick={regenerate} disabled={isGenerating}>
                        ...
                        <Button size="sm" className="flex-1" onClick={() => downloadImage(selected)}>
                    ...
                    <div className="aspect-square flex items-center justify-center border-2 border-dashed border-border rounded-lg bg-muted/20 text-center text-muted-foreground">
                      <div className="space-y-2 p-4">
                        <Megaphone className="h-10 w-10 mx-auto opacity-30" />
                        <p className="text-sm">O teu design aparecera aqui</p>
                        <p className="text-xs opacity-60">Preenche os campos e clica em Gerar</p>
                      </div>
                    </div>
                  )}
                </Card>
                ...
                <Card className="p-4 space-y-2">
                  <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Hooks de Copy Rapidos</h4>
                  <div className="flex flex-wrap gap-1.5">
                    {AI_COPY_HOOKS.map((hook) => (
                      <button
                        key={hook}
                        onClick={() => setMainText(hook + " " + mainText)}
                        className="text-xs px-2.5 py-1 rounded-full border border-border bg-muted/30 hover:bg-muted/60 hover:border-primary/50 transition-colors text-foreground"
                      >
                        {hook}
                      </button>
                    ))}
          ...
          <TabsContent value="copy" className="mt-4">
            <div className="space-y-5">
              {COPY_TEMPLATES.map(({ category, templates }) => (
                <Card key={category} className="p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {templates.map((t) => (
                      <button
                        key={t}
                        onClick={() => { setMainText(t); setActiveTab("create") }}
                        className="text-left px-4 py-3 rounded-xl border border-border bg-muted/20 hover:bg-muted/50 hover:border-primary/50 transition-all group"
                      >
                        <p className="text-sm font-medium text-foreground group-hover:text-primary transition-colors">{t}</p>
                        <p className="text-xs text-muted-foreground mt-0.5">Clica para usar como titulo</p>
                      </button>
                    ))}
                  </div>
                </Card>
          ...
          <TabsContent value="gallery" className="mt-4">
            {results.length === 0 ? (
              <Card className="p-10 text-center text-muted-foreground">
                ...
                <p className="text-sm">Ainda nao geraste nenhum design</p>
                <p className="text-xs mt-1 opacity-60">Os teus designs aparecerao aqui</p>
              </Card>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {results.map((r, i) => (
                  <button
                    key={i}
                    onClick={() => { setSelected(r); setActiveTab("create") }}
                    className="group relative aspect-square rounded-xl overflow-hidden border border-border hover:border-primary/60 transition-all"
                  >
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/50 transition-colors flex items-end p-2 opacity-0 group-hover:opacity-100">
                      <div className="flex gap-1">
                        ...
                        <Badge variant="secondary" className="text-[9px] capitalize">{r.style}</Badge>
                      </div>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); downloadImage(r) }}
                      className="absolute top-2 right-2 bg-black/70 text-white p-1.5 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/90"
                    >
